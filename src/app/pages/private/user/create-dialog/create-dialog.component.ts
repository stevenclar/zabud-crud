...
export class CreateDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.newUserFormGroup = this._formBuilder.group({
      name: [this.data?.name ?? '', [Validators.compose([Validators.required, Validators.pattern('[A-ZÑa-zñÀ-ÿ]+[A-ZÑa-zñÀ-ÿ ]*'), Validators.minLength(3), Validators.maxLength(45)])]],
      username: [this.data?.username ?? '', [Validators.compose([Validators.required, Validators.pattern('[A-ZÑa-zñÀ-ÿ]+[A-ZÑa-zñÀ-ÿ ]*'), Validators.minLength(3), Validators.maxLength(45)])]],
      email: [this.data?.email ?? '', [Validators.compose([Validators.required, Validators.pattern('[ÑA-Zña-z0-9._%+-]{3,}@[a-zA-Z]{3,}([.]{1}[a-zA-Z]{2,}|[.]{1}[a-zA-Z]{2,}[.]{1}[a-zA-Z]{2,})'), Validators.minLength(4), Validators.maxLength(70)])]],
      ...
      website: [this.data?.website ?? '', [Validators.compose([Validators.pattern('(https?://)?([\\da-z.-]+)\\.([a-z.]{2,6})[/\\w .-]*/?')])]],
    });
  ...
  getErrorMessage() {
    this.validations = {
      'name': [
        ...
        { type: 'maxlength', message: 'Máximo 45 caracteres' },
      ],
      'username': [
      ...
      'email': [
        ...
        { type: 'pattern', message: 'Se debe ingrsar un correo electrónico válido' },
        { type: 'minlength', message: 'Mínimo 4 caracteres' },
        { type: 'maxlength', message: 'Máximo 70 caracteres' },
      ],
      'phone': [
